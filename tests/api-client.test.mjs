...
test('local static runtime resolves to the Windup API service', () => {
  assert.equal(resolveApiBase({ hostname: 'localhost', port: '5173' }), 'http://127.0.0.1:5174');
  assert.equal(resolveApiBase({ hostname: '127.0.0.1', port: '5174' }), '');
});

test('asset URLs work for API paths and editor-relative catalogue paths', () => {
  const local = createApiClient('');
  const remote = createApiClient('http://127.0.0.1:5174');
  ...
  assert.equal(remote.assetUrl('/generation-data/a.png'), 'http://127.0.0.1:5174/generation-data/a.png');
});
