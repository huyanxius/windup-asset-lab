...
def _call(text, ref_paths, out_path, timeout=200, model=None):
    ...
    for attempt in range(3):                      # 空图重试（内含网络重试）
        res = provider.post_json("/chat/completions", body, timeout=timeout)
...
def to_side_view(ref_path, char_desc, out_path, model=None):
    ...
    return _call(txt, [ref_path], out_path, model=model)


def gen_character(char_desc, out_path, model=None):
    ...
    return _call(txt, [], out_path, model=model)


def gen_frame(base_path, char_desc, pose_desc, out_path, skeleton_path=None, model=None):
    ...
    return _call(txt, refs, out_path, model=model)
