...
flowchart LR
  ...
  Contract --> Candidate["角色包 / 动作条候选生成"]
  Candidate --> Process["抠图、切帧与归一化"]
  Process --> QA["自动质检"]
  QA --> Review["逐帧人工审核"]
  Review -->|退回| Candidate
...
flowchart TD
  ...
  Publisher --> Catalog["AssetCatalog"]
...
5. 动作条尺寸或可见主体不合格时允许回退逐帧；供应商鉴权或配额错误不得伪装成切帧失败。
6. 服务重启把活动 job 标成 `interrupted`，由用户明确重发。
7. 审核 409 合并当前帧意图，不能静默覆盖其他人的决定。
