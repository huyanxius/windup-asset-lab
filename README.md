...
本仓库中的 `Boy`、`Skeleton` 和 `Lirael` 角色用于验证队友管线产物的导入、展示与播放。`Boy` 只保留当前正式母版、待机与行走资产；其余角色的历史角色卡和 provenance 保留在 `artifacts/characters/`。集成适配层位于 `server/windup_pipeline/`。
...
- 快捷创建解析自然语言，并依次完成意图解析、母版、动作、质检和打包。
- 五阶段现场演示约 20 秒；生成区按真实产物到达顺序完整呈现 1 张母版、8 张 Idle 与 8 张 Walk，不允许跳过尚未到达的中间产物。完成后沿用现有的导出、发送到预览台和保存方案操作。
...
该启动方式可直接跑通角色创建、动作生成、审核与导出的完整工作流。
