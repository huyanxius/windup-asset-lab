...
| Service | Python `ThreadingHTTPServer` + Application Service | 薄 HTTP 适配、会话隔离、后台任务、审核版本、备份和溯源 |
...
├─ server/
│  ├─ app.py                     # 薄 HTTP 适配层
│  └─ windup_pipeline/           # 应用服务、会话、审核、供应商与处理模块
...
Windup 当前产品契约版本为 `1.0.0`；契约变更必须更新 `contracts/windup.v1.json` 并重新生成两端定义。
